...
include('config.php');
	...
	if(isset($_REQUEST[sub_fac]) )
	{
		if($_REQUEST['city']!="city")
		{
			...
			move_uploaded_file($_FILES[upsempic]['tmp_name'], $path1);
				echo "<script>alert('Sucessfullyセット');</script>"; 
			}else{
				echo "<script>alert('画像未満500キロバイトを設定してください');</script>"; 
			}
		}
			else{
		echo "<script>alert('都市を選択してください...！');</script>"; 
		}
	}
	...
	//echo  "<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>$_REQUEST['city']";
?>
						...
						<br><br>
									...
									<select id="allcity" class="input-name" style="width:100%!important;" name="city" onchange="">
										...
										<?php
											...
											{
										?>
												<option  value="<?php echo $fetcountry['id']; ?>"><?php echo $fetcountry['jp_name']; ?></option>
										<?php
											}
										?>
									</select>
...
状態</center></th>
									...
									?>
                                        <tr class="odd gradeX">
										<td><center><img src="../../img/<? echo $data['city_img'];?>"style="width:150px;height:100px;" class="img-responsive" /></center></td>
										<td><center><font color="black" size="5" style="font-weight:bold;margin-top:30px;"><?php $marutra = explode('"',translate(str_replace(" ","+",$data['name']))); echo $marutra[1]; ?></font></center></td>
                                           <? if($data['status']=='1'){
											?>
                                           <td><center>
