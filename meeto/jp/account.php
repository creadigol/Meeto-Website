...
if($_POST)
{
  ...
  $pwd = mysql_fetch_array(mysql_query("select * from user where id='".$uscngid."'"));
  $data_pwd=$pwd['password'];
   ...
   if($data_pwd==$old_password)
   {
	  // echo "<script>alert('hiii');</script>";
		...
		$update_pwd=mysql_query("update user set password='".$new_pass."' where id='".$uscngid."'");
		echo "<script>alert('Sucessfullyを更新します！');</script>";
		}
		else
		{
	...
	{
		echo "<script>alert('古いパスワードが間違っています！');</script>";
	}
